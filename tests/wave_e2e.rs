...
fn wave_pattern_with_tags_and_critique() {
    ...
    for title in ["X", "Y", "Z"] {
        qp(&db)
            .args(["add", title, "--tier", "wave-1", "--tag", "wave:1"])
            .assert()
            .success();
    }
    for (i, t) in [("QP-1", "a1"), ("QP-2", "a2"), ("QP-3", "a3")] {
        ...
        qp(&db)
            .args(["complete", i, "--as", t, "--decision", &format!("did {i}")])
            .assert()
            .success();
    }
    ...
    qp(&db)
        .args(["assign", &cid, "--to", "fix-1"])
        .assert()
        .success();
    qp(&db)
        .args(["claim", &cid, "--as", "fix-1"])
        .assert()
        .success();
    qp(&db)
        .args(["complete", &cid, "--as", "fix-1", "--decision", "fixed"])
        .assert()
        .success();

    // Branch-and-evaluate slice.
    let root_out = qp(&db)
        .args(["add", "explore alt", "--tag", "wave:1", "--json"])
        .assert()
        .success();
    let root: serde_json::Value = serde_json::from_str(
        std::str::from_utf8(&root_out.get_output().stdout)
            .unwrap()
            .trim(),
    )
    .unwrap();
    ...
    for v in ["a", "b", "c"] {
        let o = qp(&db)
            .args(["add", &format!("try-{v}"), "--tag", "wave:1", "--json"])
            .assert()
            .success();
        let j: serde_json::Value =
            serde_json::from_str(std::str::from_utf8(&o.get_output().stdout).unwrap().trim())
                .unwrap();
        let id = j["display_id"].as_str().unwrap().to_string();
        qp(&db)
            .args(["relation", "add", &id, "variant-of", &root_id])
            .assert()
            .success();
        variants.push(id);
    }
    ...
    for loser in &variants[1..] {
        qp(&db)
            .args(["cancel", loser, "--reason", "superseded"])
            .assert()
            .success();
    }
    qp(&db)
        .args(["relation", "add", &winner, "supersedes", &root_id])
        .assert()
        .success();
...
fn wave_block_flow_thaws_original() {
    ...
    qp(&db)
        .args(["block", "QP-1", "--new", "the blocker", "--as", "alice"])
        .assert()
        .success();
    ...
    let qp1 = arr
        ...
        .unwrap()
        .iter()
        .find(|t| t["display_id"] == "QP-1")
        .expect("QP-1 should exist");
    assert_eq!(
        qp1["state"], "pending",
        "QP-1 should be pending after block"
    );
    // Complete the blocker (QP-2).
    qp(&db)
        .args(["assign", "QP-2", "--to", "bob"])
        .assert()
        .success();
    qp(&db)
        .args(["claim", "QP-2", "--as", "bob"])
        .assert()
        .success();
    qp(&db)
        .args(["complete", "QP-2", "--as", "bob"])
        .assert()
        .success();
    ...
    assert_eq!(
        qp1["state"], "ready",
        "QP-1 should thaw to ready after blocker completes"
    );
}
