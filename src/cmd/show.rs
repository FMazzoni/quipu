...
pub fn run(db_path: &std::path::Path, a: ShowArgs) -> Result<()> {
    ...
    let mut tags: Vec<String> = tag_stmt.query_map([tid], |r| r.get::<_, String>(0))?
        .collect::<Result<_, _>>()?;
    tags.sort();
    ...
    let mut blk_stmt = conn.prepare(
        "SELECT t2.display_id FROM dep d JOIN task t2 ON t2.id = d.depends_on_task_id
          ...
          ORDER BY t2.id")?;
    let blocked_by: Vec<String> = blk_stmt.query_map([tid], |r| r.get::<_, String>(0))?
        .collect::<Result<_, _>>()?;
    ...
    let mut events: Vec<(String, String, Option<String>, Value)> = s.query_map([tid], |r| {
        ...
        Ok((r.get::<_, String>(0)?, r.get::<_, String>(1)?, r.get::<_, Option<String>>(2)?, payload_v))
    })?.collect::<Result<_, _>>()?;

    if a.json {
        // last_event is the newest (events[0] if present).
        let last_event = events.first().map(|(ts, kind, _agent, payload)| serde_json::json!({
            "kind": kind, "ts": ts, "payload": payload.clone(),
        }));
        // recent_events ordered oldest-first for friendlier consumption.
        let recent_events: Vec<Value> = events.iter().rev().map(|(ts, kind, agent, payload)| {
            serde_json::json!({
        ...
        if let Some(d) = description.as_ref() {
            obj.as_object_mut().unwrap().insert("description".into(), Value::String(d.clone()));
        }
        if let Some(c) = created_at.as_ref() {
            obj.as_object_mut().unwrap().insert("created_at".into(), Value::String(c.clone()));
        }
    ...
    let tag_summary = if tags.is_empty() { String::from("-") } else { tags.join(", ") };
