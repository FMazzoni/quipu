...
pub struct WatchArgs {
    ...
    #[arg(long, default_value_t = true)] pub json: bool,
    /// Stop after N empty-or-non-empty polling ticks. 0 = forever.
    #[arg(long, default_value_t = 0)] pub max_ticks: u64,
    /// Filter to events for this task only.
    #[arg(long)] pub task: Option<String>,
...
pub fn run(db_path: &std::path::Path, a: WatchArgs) -> Result<()> {
    ...
    let task_id = a.task.as_deref().map(|s| crate::id::resolve(&conn, s)).transpose()?;
    ...
    loop {
        ...
        let rows: Vec<(i64, Option<String>, String, String, Option<String>, Option<String>)> =
        ...
        for (id, did, ts, kind, agent, payload) in rows {
            let payload_v: serde_json::Value = payload.as_deref()
                .map(serde_json::from_str).transpose().ok().flatten()
        ...
        ticks += 1;
        if a.max_ticks > 0 && ticks >= a.max_ticks { return Ok(()); }
