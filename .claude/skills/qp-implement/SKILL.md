...
- [ ] **Every file you touch exits with a `//!` header.** One short sentence, a period, then a blank `//!` line before any detail — rustdoc uses everything before that blank line as the module-list summary, so a multi-line first paragraph renders as a wall of text. For a command module, the summary is which state-machine edge it implements (`claim` is the `assigned` → `running` edge). Reference files and function names, never line numbers. Fence any example containing `<placeholders>` as ```text or rustdoc deletes them. If a header grows past ~15 lines, keep the one-line summary in the `.rs` and move detail to `docs/modules/<name>.md` behind `#![doc = include_str!(...)]` — with a blank `//!` line before the pointer, or the summary runs on into the detail.
...
The rule above says where prose goes. Write the durable half — what the code cannot say for itself:

- **WHY** — the reason it has this shape, and what was rejected. A reader sees
  what `with_tx` does; they cannot see that read-then-write was banned
  deliberately, or that `IMMEDIATE` makes two agents racing on the same ticket
  fail fast instead of deadlocking mid-transaction.
- **INVARIANTS** — what must stay true, and what breaks when it does not. State
  the consequence: "every transition is one conditional
  `UPDATE ... WHERE state IN (...)` plus a `changes() == 1` check — a
  read-then-write here lets a concurrent claim silently win and the loser
  reports success."
- **GOTCHAS** — the thing that looks wrong but is correct, or safe but is not.
  If you had to work it out while writing the code, keep it. The blank `//!`
  line before a `#![doc = include_str!]` pointer is exactly this: invisible,
  load-bearing, and a reader will delete it.
...
`fn task_id() -> i64` is a staleness surface carrying no information.

**The test:** if this code were rewritten tomorrow in a different shape, would
the sentence still be true *and* useful? If yes, it is durable. If it only
describes the current arrangement of lines, it will rot — leave it out.
...
The `claim` records the assignment; subsequent `qp log` calls auto-attribute to you, so you don't need `--as` on log calls during your own work.
...
describes the tree being shipped. A total measured from inside your worktree
describes a tree that will not exist after the rebase.
...
target and prints nothing — you lose the unit tests and the total looks plausible
anyway. And derive the target list from `ls tests/` rather than the loop above,
which goes stale the moment someone adds a file.

Sequential, one rustc at a time, is the property the OOM rule cares about — and
what a bare `cargo test` gives up. Do not "simplify" this to `--tests`, which
fans the targets out in parallel again.

`just test` is this loop as a recipe, and `just lint` delegates its test step to
it — so `just lint` no longer ends in the bare `cargo test` this rule forbids;
running it is allowed. The prohibition stands: an unmeasured safety rule is not
relaxed because breaking it happened not to hurt.

## If your slice overturns an earlier decision

If your slice reverses an earlier call — a different default, the opposite
trade-off, a dropped convention — leave a backward reference so the reversal is
findable from the thing it reversed:
...
Afterwards the audit trail works from both ends: `qp relation list QP-<old>`
shows what replaced it, and `qp decisions` gives the filtered decision timeline.

This is a convenience, not a gate — nothing blocks without it. But an unlinked
reversal means the next agent reads the old `done` ticket and follows retired
advice. Relation kinds are free-form; `supersedes` is the one to use here.
...
The `--auto` flag marks the log entry for `qp decisions --auto-only`, which feeds the live retro via `qp timeline`. Friction notes capture what was unobvious — a surprise, a near-mistake, a place the spec was ambiguous. "Everything went smoothly" is a valid (and welcome) note when true. `qp log` needs no `--as` while you hold an active assignment.

**Those two commands are the whole list — do not tag the ticket with a branch or a commit.** Linking the ticket to where it landed is the coordinator's job: tickets are tagged `branch:<name>` at kickoff, and that tag plus the ticket's timestamps are the whole ticket↔code link (see `wave-orchestrate` Phase 4).
...
- Putting durable knowledge in repo files. Plans, critiques, sessions live in the vault (`$QUIPU_VAULT/`); bugs are qp tickets tagged `kind:bug`.
- Pulling in tokio/hyper/axum/tracing — see CLAUDE.md.
- Tagging your ticket with a branch or commit. The coordinator owns the ticket↔code link.
