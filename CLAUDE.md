...
- **`main` is protected — never push to it directly.** A GitHub ruleset (`protect-main`) rejects direct pushes server-side and requires every change to arrive through a pull request with the `lint` check green. It binds everyone, the repo owner included; there is no bypass. Land work on a branch and open a PR. Waves follow the branch/PR flow in `wave-orchestrate` ("Branch strategy" + Phase 8): the coordinator opens the PR and confirms `lint` is green, then **stops for the human to merge**. Branch names are descriptive, never a wave number or qp id — those are instance-local and meaningless on a public PR. Tickets link to code through the durable `branch:<name>` tag plus their own timestamps, not commit SHAs.
- **Never put qp ticket ids in public artifacts.** Ticket numbers are instance-local: every quipu store numbers independently, so `QP-142` refers to something different, or to nothing, in a contributor's own store. Keep them out of PR titles and descriptions, commit messages, and any prose that renders publicly — `README.md`, `docs/`, and `//!` / `///` comments. Describe the change instead of citing a number. Example placeholders in usage docs (`qp add "..."  # → QP-1`) are fine; those illustrate output rather than cite a real ticket. A ticket links to its code through the `branch:<name>` tag and its own timestamps, recorded in quipu — never through a number embedded in git or in published prose.
