...
**Tech Stack:** Rust 1.75+, `clap` v4, `rusqlite` (bundled), `serde`/`serde_json`, `time` (RFC3339), `anyhow`, `uuid` (project_uuid + hostname), `tempfile` + `assert_cmd` + `predicates` (tests).
...
| `src/main.rs` | Entry, clap, dispatch, exit-code mapping |
| `src/db.rs` | Open/migrate; `with_tx`; `guarded_update`; `refresh_ready`; `now`; `insert_event`; `has_path` (cycle check); `host`; constants |
...
**Files:** `src/schema.sql`, `src/db.rs`, `src/main.rs`, `tests/cli.rs`

- [ ] **Step 1: Write `src/schema.sql`** — paste the full Schema block from the header above (`PRAGMA`s, 7 tables, 9 indexes).
...
- [ ] **Step 4: Write `src/db.rs`** — full file:
...
pub fn resolve_path(explicit: Option<PathBuf>) -> Result<PathBuf> {
    if let Some(p) = explicit.clone() { return Ok(p); }
...
/// Constraint-error type — exit code 2.
#[derive(Debug)]
...
pub fn constraint(msg: impl Into<String>) -> anyhow::Error { anyhow!(ConstraintError(msg.into())) }
```
...
fn main() {
    if let Err(e) = real_main() {
        eprintln!("error: {e:#}");
        if e.downcast_ref::<db::ConstraintError>().is_some() { std::process::exit(2); }
        std::process::exit(1);
    }
}
...
- [ ] **Step 6: Run** — first test PASSes. Second test stays `#[ignore]` until Task 14.

- [ ] **Step 7: Commit**

```bash
git add src/schema.sql src/db.rs src/main.rs tests/cli.rs
git commit -m "feat(db): schema (7 tables) + open/migrate + project_uuid + guards"
```
